...
# pinned `VAR="${VAR:-value}"` default from common/helper
pin() { sed -n "s/^$1=\"\${$1:-\(.*\)}\"\$/\1/p" "$HELPER"; }
# pinned `  KEY: value` from ci.yml
ci_pin() { sed -n "s/^[[:space:]]*$1:[[:space:]]*\(.*\)\$/\1/p" "$CI" | head -1; }

# rewrite VAR's default in common/helper, in place. `cat >` rewrites the existing file's
# contents so its mode (helper is 0755) is preserved - unlike `mv tmp file`, which wouldn't.
set_pin() {
    local var=$1 val=$2 tmp; tmp=$(mktemp)
    awk -v v="$var" -v n="$val" 'index($0,v"=\"${"v":-")==1{sub(/:-[^}]*}/,":-"n"}")}1' "$HELPER" >"$tmp"
    cat "$tmp" >"$HELPER"; rm -f "$tmp"
}
...
check() {
    ...
    check_one s6-overlay "$(pin S6_OVERLAY_VERSION)"  "$(strip_v "$(gh_latest just-containers/s6-overlay)")"
    check_one composer   "$(pin COMPOSER_VERSION)"    "$(composer_latest)"
    check_one pie        "$(pin PIE_VERSION)"         "$(strip_v "$(gh_latest php/pie)")"
    check_one castor     "$(pin CASTOR_VERSION)"      "$(strip_v "$(gh_latest jolicode/castor)")"
    check_one goss       "$(strip_v "$(ci_pin GOSS_VERSION)")" "$(strip_v "$(gh_latest goss-org/goss)")"
    check_one gcloud     "$(pin GCLOUD_VERSION)"      "$(gcloud_latest)"
...
refresh_s6() {
    local v base d
    v=$(pin S6_OVERLAY_VERSION)
    base="https://github.com/just-containers/s6-overlay/releases/download/v${v}"
    d=$(fetch "$base/s6-overlay-noarch.tar.xz");  set_pin S6_OVERLAY_SHA256_NOARCH "$(sha256 "$d")"; rm -f "$d"
    d=$(fetch "$base/s6-overlay-x86_64.tar.xz");  set_pin S6_OVERLAY_SHA256_AMD64  "$(sha256 "$d")"; rm -f "$d"
    d=$(fetch "$base/s6-overlay-aarch64.tar.xz"); set_pin S6_OVERLAY_SHA256_ARM64  "$(sha256 "$d")"; rm -f "$d"
    echo "  s6-overlay $v: refreshed"
}
refresh_composer() {
    local v d; v=$(pin COMPOSER_VERSION)
    d=$(fetch "https://getcomposer.org/download/${v}/composer.phar"); set_pin COMPOSER_SHA256 "$(sha256 "$d")"; rm -f "$d"
    echo "  composer $v: refreshed"
}
refresh_pie() {
    local v d; v=$(pin PIE_VERSION)
    d=$(fetch "https://github.com/php/pie/releases/download/${v}/pie.phar"); set_pin PIE_SHA256 "$(sha256 "$d")"; rm -f "$d"
    echo "  pie $v: refreshed"
}
refresh_castor() {
    local v d; v=$(pin CASTOR_VERSION)
    d=$(fetch "https://github.com/jolicode/castor/releases/download/v${v}/castor.linux-amd64"); set_pin CASTOR_SHA256_AMD64 "$(sha256 "$d")"; rm -f "$d"
    d=$(fetch "https://github.com/jolicode/castor/releases/download/v${v}/castor.linux-arm64"); set_pin CASTOR_SHA256_ARM64 "$(sha256 "$d")"; rm -f "$d"
...
refresh_gcloud() {
    local v d; v=$(pin GCLOUD_VERSION)
    echo "  gcloud $v: downloading tarballs (~150MB each) ..."
    d=$(fetch "https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-${v}-linux-x86_64.tar.gz"); set_pin GCLOUD_SHA256_AMD64 "$(sha256 "$d")"; rm -f "$d"
    d=$(fetch "https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-${v}-linux-arm.tar.gz");    set_pin GCLOUD_SHA256_ARM64 "$(sha256 "$d")"; rm -f "$d"

...
- `common/`: shared installers, PHP/FPM/dev configuration, health probe, and sample page.
- `<image>/`: Dockerfile, server/service configuration, and goss suites.
- `examples/`: Symfony, Laravel, Nette, and WordPress Compose examples; see
  `examples/README.md` and `examples/Makefile` for setup.
- `.github/workflows/ci.yml`: build/test/publish matrix and release manifests.
- `.github/workflows/deps-check.yml`, `.github/renovate.json`, and
  `.github/dependabot.yml`: dependency maintenance.
- `Makefile`: authoritative local targets and build defaults.

## Naming and tags

See `docs/tags.md` for the authoritative scheme. Summary:

- The web images ship Debian and Alpine variants; `dind` is Alpine-only.
- CI produces multi-arch images (`linux/amd64` + `linux/arm64`).
...
- Tag format is `[<version>-]<os>` (`<os>` = `debian` | `alpine`):
  ...
  - Dev variant of the web images: `<php-version>-<os>-dev` (e.g. `8.5-alpine-dev`);
    `-dev` names the trait (adds the dev toolbox), same as `-rootless` for dind. See
    the Dev image variant section.
- CI publishes to `ghcr.io/<owner>/<image>` on pushes to `main`. Docker Hub mirroring
  is optional via `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN`. Local image names have
  no registry prefix unless `REGISTRY` is set (include its trailing slash).
...
- Debian vs Alpine is a build-time input, not a separate file:
  ...
  - Each script follows a fixed shape (borrowed from the internal php-base `alfred-*`
    toolbox): a description + `# Usage:` header, then input validation
...
- Keep comments concise: explain non-obvious constraints or rationale, not what the
  next command does. Put detailed setup and architecture explanations in `docs/`.
  Preserve shebangs, Dockerfile section markers, Renovate/ShellCheck directives,
  script usage headers, Make help annotations, and useful commented configuration examples.
...
- `jarvis-install-runtime-deps <pkgs...>` -> apt-get or apk, with cache cleanup.
  Debian accepts native APT patterns (e.g. `~n^libicu[0-9]+$`) for ABI-versioned
  runtime packages; declare them in `runtime_deps` and install before build deps.
  Quote direct pattern arguments; use `set -f` when expanding a whitespace-separated
  package list so the shell does not expand patterns against local filenames.
...
- `jarvis-install-docker-ext` -> `docker-php-ext-install` for extensions bundled with PHP.
  A quoted argument such as `"gd --with-freetype --with-jpeg --with-webp"` runs
  `docker-php-ext-configure` first. Specs split on whitespace; flag values cannot
  contain spaces. Plain extension names retain their existing behaviour.
- `jarvis-install-pie-ext` -> `pie install` for external extensions (`vendor/name`, optionally `vendor/name:version`)
- `jarvis-install-pecl-ext` -> legacy fallback (`pecl install` + `docker-php-ext-enable`)

Prefer installers in that order: bundled PHP source, then PIE, then PECL only when
PIE cannot provide the required extension/version. Document any PECL fallback.
The current production and dev Dockerfiles need no PECL installer calls. PIE
packages under the `pecl/` namespace (such as `pecl/pcov`) still use PIE.
...
Downloaded artifacts are **sha256-verified** before use (via the shared
`jarvis-verify-sha256` command), against digests **pinned in the same `jarvis-install-*` file as the
...
```sh
make build              # prod web images (both OS variants) + plain dind
make <image>            # e.g. make fpm-nginx (both variants)
make <image>-<os>       # e.g. make fpm-nginx-alpine (single variant)
make dev                # dev web images, both OS variants
make dind-aws           # dind with AWS CLI (also dind-gcloud / dind-azure)
make test-fpm-nginx      # build + runtime-test the Alpine prod image
make test-dev-fpm-nginx  # build + runtime-test the Alpine dev image
make test-dind-aws       # build + runtime-test a cloud variant
make help               # list annotated aggregate targets
```

Variables: `PHP_VERSION` (Makefile default 8.5), `DOCKER_VERSION` (default 29),
`REGISTRY` (tag prefix), `PLATFORM` (empty = host arch), `OUTPUT` (default `--load`),
and `CACHE` (additional buildx cache flags). The web Dockerfiles default to PHP 8.5
when built directly; the CI matrix also selects PHP 8.5. Keep these defaults aligned
when changing the supported PHP version.

Local builds load one platform into Docker. Multi-platform publishing is supported
by setting `OUTPUT=--push PLATFORM=linux/amd64,linux/arm64 REGISTRY=<registry>/`.
`make build` excludes dev and cloud variants; use their separate targets above.
...
- `build-php-image` job: matrix of `arch` (amd64 / arm64) x `os` (alpine / debian) x
  **`variant` (prod / dev)** x `target` (targets x 2 x 2 x 2 jobs). Each runs on the
  ...
  below). `IMAGE`/`PHP`/`OS`/`VARIANT`/`MAKE_TARGET`/`TAG`/`GHCR_REF`/`DOCKERHUB_REF` are job-level `env`
...
- Push runs after build and runtime tests (and graceful-stop checks for web images),
  on main only, gated by `if: github.event_name == 'push' && github.ref == 'refs/heads/main'`; the
...
- **nginx (`fpm-nginx`):** `nginx.conf` already used `/dev/stdout` (access) + `/dev/stderr`
  ...
  `${SERVER_TIMEOUT}` (nginx can't expand env itself).
...
- **PHP extensions:** all three web images install the same defaults with explicit
  `jarvis-*` calls. Bundled extensions: `mysqli`, `pdo_mysql`, `pdo_pgsql`, `bcmath`,
  `intl`, `zip`, `gd`, `exif`, and `pcntl`. PIE installs `igbinary/igbinary`,
  `imagick/imagick`, `phpredis/phpredis`, and `apcu/apcu`, without version pins in
  the Dockerfiles.
  `igbinary/igbinary:@RC` allows release candidates because PIE currently has no
  compatible stable release.
  `ds` is not installed.
  - Install runtime packages before removable build dependencies. PostgreSQL, ICU,
    ZIP, GD, and ImageMagick need external libraries. On Debian, retain installed
    ICU/ZIP/PNG runtime packages by querying dpkg, since Bookworm and Trixie use
    different package names. Remove headers and build tools with
    `jarvis-remove-build-deps` after compilation.
  - GD is configured with FreeType, JPEG, and WebP support; PNG is enabled by default.
    Alpine needs the ImageMagick JPEG/WebP coder packages in addition to the core
    runtime. Ghostscript is not included, so PDF thumbnail generation is not promised.
  - Production and dev goss suites check enabled extensions with a single `php -m`
    command and exact module-name matches. Dev suites also include Xdebug, PCOV,
    and SPX in that list.
  - `igbinary` is installed, but applications must choose their serializer. Installing
    it does not automatically enable optional igbinary support in other extensions.
  - The dev stages additionally install Composer, Castor, Xdebug, PCOV, and SPX.
    The WordPress example inherits all runtime extensions without reinstalling GD.
  - Downstream applications can add more extensions by deriving from an image and
    using `jarvis-install-docker-ext`, then `jarvis-install-pie-ext`, with
    `jarvis-install-pecl-ext` only as a fallback. Run installers as root, then
    restore `USER ${SERVER_USER}`.
...
- **Extensions installed explicitly.** The `dev` stage runs `jarvis-install-composer`,
  `jarvis-install-castor`, then `jarvis-install-pie-ext xdebug/xdebug pecl/pcov
  noisebynorthwest/php-spx` (no version constraints; `jarvis-install-pie-ext` installs the
...
- **Per-distro packages via a `jarvis-detect-os` case.** The `RUN` sets two shell vars:
  `build_deps` (removable) - `$PHPIZE_DEPS unzip libtool` (the pecl/pie toolchain) plus the headers
...
The workflow defines build and runtime checks for all image variants; this file is
not a record of the latest passing CI run. Report only checks actually run for the
current change, including the image, PHP version, OS, and architecture tested.

For runtime changes, build and run the relevant goss suite. Shared toolbox or PHP
configuration changes can affect all three web images and both OS variants. Local
`make test` covers Alpine prod images and plain dind; `make test-dev` and
`make test-dind-<cloud>` cover the additional variants. Debian testing follows the
CI pattern: build the Debian tag and run dgoss from the image directory.
Documentation-only changes need link/path checks and `git diff --check`, not image
rebuilds.
